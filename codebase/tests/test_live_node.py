...
def _result(action=(0.0, 0.0, 0.0)):
    return PerceptionResult(action, PerceptionClaim.from_detections([]))
...
def test_perception_failure_clears_previous_snapshot():
    source = StaticSource(np.zeros((8, 8, 3), dtype=np.uint8))
    worker = PerceptionWorker(source, lambda _frame: _result())
...
def test_invalid_action_never_becomes_a_snapshot():
    ...
    worker = PerceptionWorker(
        source,
        lambda _frame: _result((float("nan"), 0.0, 0.0)),
    )
    assert worker.sample_once() is None
    assert worker.snapshot() is None
    assert "action must contain" in worker.last_error
...
def test_action_only_inference_never_becomes_a_snapshot():
    source = StaticSource(np.zeros((8, 8, 3), dtype=np.uint8))
    worker = PerceptionWorker(source, lambda _frame: (0.0, 0.0, 0.0))
    assert worker.sample_once() is None
    assert "PerceptionResult" in worker.last_error
