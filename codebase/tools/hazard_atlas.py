...
def hazard_atlas_html(asset_root: Path, google_key: str = "") -> bytes:
    available = {
        name
    ...
    return (
        HAZARD_TEMPLATE.replace("__REFERENCE_CARDS__", "".join(cards))
        .replace("__GOOGLE_KEY__", json.dumps(google_key.strip()))
        .encode("utf-8")
    )
...
const googleKey=__GOOGLE_KEY__;let map=null,info=null,selected=regions[0];const groups={};
function loadGoogle(){return new Promise((resolve,reject)=>{if(window.google&&google.maps)return resolve();if(!googleKey)return reject(new Error('Google Maps API key is not configured'));const old=document.getElementById('google-maps-sdk');if(old){old.addEventListener('load',resolve,{once:true});old.addEventListener('error',reject,{once:true});return}const s=document.createElement('script');s.id='google-maps-sdk';s.async=true;s.defer=true;s.src=`https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(googleKey)}`;s.onload=resolve;s.onerror=()=>reject(new Error('Google Maps failed to load'));document.head.appendChild(s)})}
function regionCenter(r){if(r.shape==='circle')return{lat:r.coords[0],lng:r.coords[1]};const total=r.coords.reduce((a,p)=>[a[0]+p[0],a[1]+p[1]],[0,0]);return{lat:total[0]/r.coords.length,lng:total[1]/r.coords.length}}
function selectRegion(r,layer){selected=r;document.getElementById('tier').textContent=r.tier;document.getElementById('regionName').textContent=r.name;document.getElementById('regionPlace').textContent=r.place.toUpperCase();document.getElementById('regionWhy').textContent=r.why;document.getElementById('checklist').innerHTML=r.need.map(x=>`<li>${x}</li>`).join('');if(map&&info){info.setContent(`<b>${r.name}</b><br>${r.place}`);info.setPosition(regionCenter(r));info.open({map});map.panTo(regionCenter(r))}}
async function initMap(){try{await loadGoogle()}catch(error){document.getElementById('map').innerHTML=`<div style="padding:30px;color:#ffb9b9"><b>Google Maps unavailable.</b><br>${error.message}</div>`;return}map=new google.maps.Map(document.getElementById('map'),{center:{lat:22.7,lng:79.2},zoom:5,mapTypeId:'roadmap',mapTypeControl:false,streetViewControl:false,fullscreenControl:true,restriction:{latLngBounds:{north:38.5,south:5.0,west:66.0,east:99.5},strictBounds:false}});info=new google.maps.InfoWindow();for(const key of Object.keys(layerInfo))groups[key]=[];regions.forEach(r=>{const common={map,strokeColor:colors[r.type],strokeOpacity:.95,strokeWeight:r.type==='priority'?3:2,fillColor:colors[r.type],fillOpacity:r.type==='priority'?.38:.18,clickable:true};const layer=r.shape==='circle'?new google.maps.Circle({...common,center:regionCenter(r),radius:r.radius}):new google.maps.Polygon({...common,paths:r.coords.map(p=>({lat:p[0],lng:p[1]}))});layer.addListener('click',()=>selectRegion(r,layer));groups[r.type].push(layer);r.layer=layer});selectRegion(regions[0],regions[0].layer)}
for(const [key,[title,sub]] of Object.entries(layerInfo)){const row=document.createElement('div');row.className='layer on';row.dataset.layer=key;row.innerHTML=`<span class="swatch" style="background:${colors[key]}"></span><span><strong>${title}</strong><small>${sub}</small></span><span class="switch"></span>`;row.onclick=()=>{row.classList.toggle('on');const on=row.classList.contains('on');for(const overlay of (groups[key]||[]))overlay.setMap(on?map:null)};document.getElementById('layers').appendChild(row)}
document.querySelectorAll('[data-base]').forEach(btn=>btn.onclick=()=>{document.querySelectorAll('[data-base]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');if(map)map.setMapTypeId(btn.dataset.base==='satellite'?'hybrid':'roadmap')});initMap();
