...
7. **Unreal content is organized as scenario packs, not one co-edited map.** Pratik's
   primary scenario is the dark urban / night-mode world with buildings, rooftops, and
   physical 3-D adversarial artifacts on rooftops or walls. A light-terrain variant
   with hills, mountains, desert, lakes, and trees may be used as secondary robustness
   evidence only after the same pinned-world, replay, safety, and artifact gates pass.
...
6. For environment handoffs, Abhijan validates both clean and attacked frame sets: patch
   lowered, patch raised, vehicle-to-node mapping, RGB/depth/pose metadata, detector
   outputs, action deltas, expected invariant, and reason code. The same validator must
   work for Pratik's dark urban pack and any accepted light-terrain variant.
...
2. Build a bounded five-vehicle world with a close, frame-relevant obstacle, collision
   detection, known dimensions, and independent ground-truth logging. The primary visual
   target is the dark urban / night-mode rooftop-patch scenario; keep it simple enough to
   reproduce before adding visual detail.
...
5. If a light-terrain world is added, treat it as a separate exported scenario pack with
   its own hashes, seeds, calibration, and replay data. Do not merge unrelated Unreal
   binary assets through ad hoc Git conflict resolution.
...
5. A light-terrain variant may be produced on Samik's installation only as a secondary
   scenario pack after command-adapter, failsafe, reset, and evidence duties are passing.
   It must not replace the independent reproduction of Pratik's primary world.
...
5. **Optional robustness scene:** replay the accepted light-terrain scenario pack through
   the same verifier/dashboard path if it passed the evidence gates. Present it as a
   second environment, not as a stitched multi-world swarm.
6. **Reveal and limits:** show the retained run bundle, explain that OP-TEE protects the
